...
namespace Service_center
{
    public partial class Registration : Form
    {
        ...
        public Registration(bool check)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT * FROM Employee WHERE Login = @Login", connection))
                    {
                        cmd.Parameters.AddWithValue("@Login", textBox1.Text);
                        if (cmd.ExecuteScalar() == null)
                        {
                            try
                            {
                                using (SqlCommand cmd1 = new SqlCommand(@"INSERT INTO [Employee] (Login, Password, Name, Surname, Patronymic) VALUES (@Login, @Password, @Name, @Surname, @Patronymic)", connection))
                                {
                                    cmd1.Parameters.AddWithValue("@Login", textBox1.Text);
                                    cmd1.Parameters.AddWithValue("@Name", textBox2.Text);
                                    cmd1.Parameters.AddWithValue("@Surname", textBox3.Text);
                            ...
                            {
                                MessageBox.Show("Успешная регистрация!", "Аккаунт сотрудника создан", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                connection.Close();
                                Close();
                            }
                        }
